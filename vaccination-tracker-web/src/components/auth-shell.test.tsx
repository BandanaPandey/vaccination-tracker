...
describe("AuthShell", () => {
  ...
  it("renders authenticated profile management after successful login", async () => {
  ...
  it("adds, edits, deletes, and switches profiles", async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByText(/aarav pandey/i).length).toBeGreaterThan(0);
    });
    ...
    await waitFor(() => {
      expect(screen.getAllByText(/aarav p\./i).length).toBeGreaterThan(0);
    });
    ...
    await waitFor(() => {
      expect(screen.queryByText(/aarav p\./i)).not.toBeInTheDocument();
    });
  });
});
